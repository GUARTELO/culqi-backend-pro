...
// ================================================================
// Ubicación: src/services/chatbotService.js
// ================================================================

const admin = require('firebase-admin');
const nodemailer = require('nodemailer');
const { ChatbotSession, normalizePhone } = require('../models/chatbotSession');

const CONFIG = {
    COMPANY_NAME: 'Brioni',
    COMPANY_EMAIL: 'contacto@goldinfiniti.com',
    COMPANY_PHONE: '+51968786648',
    COMPANY_WHATSAPP: '51968786648',
    ADMIN_EMAIL: 'admin@goldinfiniti.com',
    DOMAIN: 'https://www.goldinfiniti.com'
};

// ============================================================
// PROCESAR MENSAJE - PRINCIPAL
// ============================================================
async function processMessage(message, phone, channel = 'web') {
    try {
        const cleanText = message.trim().toLowerCase();
        const normalizedPhone = normalizePhone(phone);
        
        console.log(`[Chatbot] Mensaje de ${normalizedPhone} (${channel}): "${cleanText}"`);

        const session = new ChatbotSession(normalizedPhone, channel);
        ...
        // 1. COMANDOS BÁSICOS
        // ============================================================
        if (['hola', 'menu', 'inicio', 'ayuda', 'start'].includes(cleanText)) {
        ...
        // 2. CONSULTAR PEDIDO
        // ============================================================
        if (cleanText.includes('pedido') || 
            cleanText.includes('orden') || 
            cleanText.includes('seguimiento') ||
            cleanText.includes('estado')) {
            
            const match = cleanText.match(/(br-?\d{4}-?\d{4})/i) || 
                           cleanText.match(/(\d{4}-?\d{4})/);
            
            if (match) {
                const orderNumber = match[1].toUpperCase();
                await session.updateContext({ orderId: orderNumber, waitingFor: null });
                await session.setStep('order_tracking');
                return await getOrderStatus(orderNumber);
            } else {
                await session.setStep('waiting_order');
                await session.updateContext({ waitingFor: 'order_number' });
        ...
        // ============================================================
        if (userSession.step === 'waiting_order' || userSession.context?.waitingFor === 'order_number') {
            const match = cleanText.match(/(br-?\d{4}-?\d{4})/i) || 
                           cleanText.match(/(\d{4}-?\d{4})/);
            ...
            if (match) {
                ...
                return "❌ No entendí el número de pedido.\n\nDebe ser como: *BR-2026-1234*\n\n_Escribe 'menu' para volver_";
            }
        }
        ...
        // ============================================================
        // 8. ASESOR (ABRE WHATSAPP + NOTIFICA ADMIN)
        // ============================================================
        if (cleanText.includes('asesor') || 
            ...
            await session.setStep('menu');
            return await transferToHuman(normalizedPhone);
        }
        ...
        if (cleanText.length > 3) {
            const result = await searchProducts(cleanText);
            if (!result.includes('No encontré')) {
                await session.updateContext({ lastQuery: cleanText });
                return result;
            }
        }
        ...
        console.error('[ChatbotService] Error en processMessage:', error);
        return '⚠️ Ocurrió un error. Por favor, escribe "menu" para volver a empezar.';
    }
}

// ============================================================
// CONSULTAR PEDIDO EN FIREBASE (SOLO LECTURA)
// ============================================================
async function getOrderStatus(orderNumber) {
    try {
        console.log(`[Chatbot] Consultando pedido: ${orderNumber}`);

        // SOLO LECTURA - Firebase (colección 'ordenes' como en tu sistema)
        const ordersRef = admin.firestore().collection('ordenes');
        ...
        const estado = order.estado || order.status || 'pending';
        
        let response = `📦 *ESTADO DE PEDIDO*\n\n`;
        response += `🔢 Orden: ${order.numeroOrden || order.id}\n`;
        response += `📊 Estado: ${statusMap[estado] || estado}\n`;
        response += `💰 Total: S/${order.resumen?.total || 0}\n`;
        response += `📦 Items: ${order.productos?.length || 0}\n`;

        if (order.envio?.guia) {
            response += `📮 Guía: ${order.envio.guia}\n`;
        }
        ...
        console.error('[ChatbotService] Error en getOrderStatus:', error);
        return '⚠️ Error al consultar el pedido. Intenta de nuevo más tarde.';
    }
}

// ============================================================
// BUSCAR PRODUCTOS EN FIREBASE (SOLO LECTURA)
// ============================================================
async function searchProducts(query) {
    try {
        console.log(`[Chatbot] Buscando productos: "${query}"`);

        // SOLO LECTURA - Firebase (colección 'productos' como en tu sistema)
        ...
        for (let i = 0; i < maxResults; i++) {
            ...
            response += `   ${categoryEmoji} ${p.categoria || 'General'} | 💰 S/${p.precio || 0}\n`;
            response += `   📦 ${status}`;
            if (stock > 0 && stock < 5) {
                response += ` ⚠️ ¡Últimas ${stock} unidades!`;
            }
            response += `\n\n`;
        }

        if (results.length > 5) {
            response += `_Y ${results.length - 5} productos más..._\n\n`;
        }

        response += `🌐 Ver todos: ${CONFIG.DOMAIN}/search?q=${encodeURIComponent(query)}\n\n`;
        response += `_Responde con el número del producto para más detalles_`;
        return response;

    } catch (error) {
        console.error('[ChatbotService] Error en searchProducts:', error);
        return '⚠️ Error al buscar productos. Intenta de nuevo.';
    }
}

// ============================================================
// TRANSFERIR A ASESOR - ABRE WHATSAPP + NOTIFICA ADMIN
// ============================================================
async function transferToHuman(phone) {
    try {
        console.log(`[Chatbot] Transferiendo a asesor: ${phone}`);
        ...
        await transporter.sendMail({
            ...
            subject: '👤 Solicitud de atención - Chat Asistente Brioni',
            html: `
                <!DOCTYPE html>
                <html>
                <head>
                    <style>
                        ...
                        .header { background: #075E54; color: white; padding: 15px; border-radius: 8px 8px 0 0; }
                        .content { background: #f5f5f5; padding: 20px; border-radius: 0 0 8px 8px; }
                        .info { background: white; padding: 15px; border-radius: 8px; margin: 10px 0; }
                        .btn { display: inline-block; background: #25D366; color: white; padding: 10px 20px; border-radius: 5px; text-decoration: none; }
                    </style>
                </head>
                <body>
                    <div class="container">
                        <div class="header">
                            <h2>👤 Cliente solicita atención personalizada</h2>
                        </div>
                        <div class="content">
                            <div class="info">
                                <p><strong>📱 Teléfono:</strong> ${phone || 'No disponible'}</p>
                                <p><strong>📅 Fecha:</strong> ${new Date().toLocaleString('es-PE')}</p>
                                <p><strong>🌐 Origen:</strong> Chat web Brioni</p>
                            </div>
                            <p style="margin-top: 20px;">
                                <a href="https://wa.me/${phone || CONFIG.COMPANY_WHATSAPP}" class="btn" target="_blank">
                                    💬 Responder por WhatsApp
                                </a>
                            </p>
                            <p style="font-size: 12px; color: #888; margin-top: 20px;">
                                Este mensaje fue generado automáticamente por el asistente virtual Brioni.
                            </p>
                        </div>
                    </div>
                </body>
        ...
        console.log('[ChatbotService] Email de notificación enviado');

        // 2. Mensaje para el usuario con enlace a WhatsApp
        ...
        console.error('[ChatbotService] Error en transferToHuman:', error);
        return `👤 *ATENCIÓN PERSONALIZADA*\n\n` +
