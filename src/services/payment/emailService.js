...
async function _generateOrderPDF(firebaseData) {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (fecha_creacion?._seconds) fechaOrden = new Date(fecha_creacion._seconds * 1000);
      if (typeof fecha_creacion === 'string') fechaOrden = new Date(fecha_creacion);

      const fecha = fechaOrden.toLocaleDateString('es-PE', { timeZone: 'America/Lima' });
      const hora = fechaOrden.toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit' });

      /* =========================
         CALCULAR ALTURA DINÁMICA
      ========================= */
      const ALTURA_BASE = 360;
      const ALTURA_PRODUCTO = 42;
      const ALTURA_DETALLE = 10;

      let alturaProductos = 0;

      productos.forEach(p => {
        alturaProductos += ALTURA_PRODUCTO;
        if (p.color || p.talla || p.sku) alturaProductos += ALTURA_DETALLE;
      });

      const ALTURA_TOTAL = ALTURA_BASE + alturaProductos;

      /* =========================
         DOCUMENTO TICKET
      ========================= */
      const doc = new PDFDocument({
        size: [226, ALTURA_TOTAL],
        margin: 20,
        info: {
      ...
      /* =========================
         HEADER
      ========================= */
      doc.font('Helvetica-Bold').fontSize(14).text('GOLDINFINITI', 0, y, center);
      y += 18;

      doc.fontSize(8).font('Helvetica').text('Comprobante de compra', 0, y, center);
      y += 14;

      doc.moveTo(20, y).lineTo(206, y).stroke();
      y += 10;
      ...
      ========================= */
      doc.fontSize(8);
      doc.text(`Orden: ${order_id}`, 20, y); y += 12;
      doc.text(`Fecha: ${fecha}`, 20, y); y += 12;
      doc.text(`Hora: ${hora}`, 20, y); y += 14;
      ...
      /* =========================
         CLIENTE
      ========================= */
      doc.font('Helvetica-Bold').text('Cliente', 20, y); y += 12;
      doc.font('Helvetica');
      doc.text(`${cliente.nombre} ${cliente.apellido}`, 20, y); y += 12;
      doc.text(cliente.email, 20, y, { width: 180 }); y += 14;

      /* =========================
         PRODUCTOS
      ========================= */
      doc.font('Helvetica-Bold').text('Detalle', 20, y); y += 12;
      doc.font('Helvetica');
      ...
      productos.forEach(p => {
        ...
        doc.text(nombre, 20, y, { width: 180 });
        y += 12;

        if (p.color || p.talla || p.sku) {
          const detalles = [];
          if (p.color) detalles.push(`Color: ${p.color}`);
          if (p.talla) detalles.push(`Talla: ${p.talla}`);
          if (p.sku) detalles.push(`SKU: ${p.sku}`);

          doc.fontSize(7).fillColor('#666')
             .text(detalles.join(' | '), 20, y, { width: 180 });
          doc.fontSize(8).fillColor('#000');
          y += 10;
        }

        doc.text(`${cantidad} x S/ ${precio.toFixed(2)}`, 20, y);
        doc.text(`S/ ${subtotal.toFixed(2)}`, 20, y, right);
        y += 14;
      });
      ...
      doc.text(`S/ ${resumen.subtotal.toFixed(2)}`, 20, y, right);
      y += 12;

      if (envio?.costo > 0) {
        doc.text('Envío', 20, y);
        doc.text(`S/ ${envio.costo.toFixed(2)}`, 20, y, right);
        y += 12;
      }
      ...
      doc.text('TOTAL', 20, y);
      doc.text(`S/ ${resumen.total.toFixed(2)}`, 20, y, right);
      y += 20;
      ...
      doc.text('Pago aprobado ✔', 0, y, center); y += 12;
      doc.text('Gracias por tu compra', 0, y, center); y += 12;
      doc.fontSize(7).fillColor('#777');
      doc.text(`ID: ${order_id}`, 0, y, center);
